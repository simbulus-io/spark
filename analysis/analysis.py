...
json_df = (
    pd.json_normalize(flat_events, meta=["_id.$oid"])
...
prepped_df = pd.merge(equivacards_events_df, mapping_df, how="left")
...
prepped_df[column_order].sort_values("timestamp").to_csv(
    os.path.join(output_dir, "enriched_events.csv"), index=False
)

turn_metrics_df.sort_values("turn_start_time_et").to_csv(
    os.path.join(output_dir, "turn_level_metrics.csv"), index=False
)
