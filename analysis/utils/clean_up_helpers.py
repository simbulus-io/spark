...
def align_timestamps(input_df):
    ...
    output_df["unix_timestamp_combined"] = output_df.apply(
        lambda x: align_unix_convention(x), axis=1
    )
...
def add_game_turn_boundary_flags(input_df):
    equiv_events_df = input_df.sort_values(by="timestamp").copy()
    ...
    equiv_events_df["game_start_time"] = equiv_events_df.apply(
        lambda x: x.timestamp if x.game_start else None, axis=1
    )

    equiv_events_df["corrected_activity"] = equiv_events_df.event_name.apply(
        correct_launch_activity
    )
    equiv_events_df["game_end"] = equiv_events_df.event_name.apply(
        lambda x: x in ["user_won", "user_lost"]
    )
    equiv_events_df["game_end_time"] = equiv_events_df.apply(
        lambda x: x.timestamp if x.game_end else None, axis=1
    )
    equiv_events_df["user_turn_start"] = equiv_events_df.event_name.apply(
        lambda x: x in ["user_turn"]
    )
    equiv_events_df["user_turn_start_time"] = equiv_events_df.apply(
        lambda x: x.timestamp if x.user_turn_start else None, axis=1
    )
    ...
    equiv_events_df["user_turn_end_time"] = equiv_events_df.apply(
        lambda x: x.timestamp if x.user_turn_end else None, axis=1
    )
    ...
    equiv_events_df["user_action_time"] = equiv_events_df.apply(
        lambda x: x.timestamp if x.user_took_action else None, axis=1
    )
    equiv_events_df["user_launch_index"] = equiv_events_df.groupby(
        "user_id"
    ).game_launch.cumsum()
    equiv_events_df["user_game_index"] = equiv_events_df.groupby(
        "user_id"
    ).game_start.cumsum()
    ...
    return corrected_activity_events_df.drop(["activity"], axis=1, errors="ignore")
